...
R=3;
f=2.4;
load(['output_',num2str(f),'GHz/design',num2str(design),'_noise=',num2str(noise),'_gdop=',num2str(gdop),'_R=',num2str(R),'.mat'])
%load(['output_',num2str(f),'GHz/design',num2str(design),'_noise=',num2str(noise),'_gdop=',num2str(gdop),'.mat'])

load(['noise=',num2str(noise),'/R=',num2str(R),'_gdop=',num2str(gdop),'_noise=',num2str(noise),'_ga_output.mat'])


% Sorting the resulting function values based on beacon count -> row 1
...
fprintf('\n%d\t%d\t%d\t%d\t%.0f\t%f\t%f\t%f\n',design-1,gdop,R,noise*100,sol_count(final),sol_density(final),sol_coverage(final),sol_error(final));
fprintf('\n%d\t%d\t%d\t%d\t%.0f\t%f\t%f\t%f\n',...
design-1,gdop,R,noise*100,f(1),-f(2),final_solve_mat(1),final_error_mat(1));
